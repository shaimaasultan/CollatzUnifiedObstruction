import networkx as nx
import matplotlib.pyplot as plt
from collections import deque, defaultdict


def preimages(y):
    res = []

    # even preimage: always valid
    x_even = 2 * y
    res.append(x_even)

    # odd preimage: only if (y - 1) divisible by 3 and result is odd
    if (y - 1) % 3 == 0:
        x_odd = (y - 1) // 3
        if x_odd % 2 == 1 and x_odd > 0:
            res.append(x_odd)

    return res


def hierarchical_layout(G, roots):
    """Produce a clean layered layout."""
    pos = {}
    layers = defaultdict(list)
    visited = set()
    q = deque()

    # Initialize roots at depth 0
    for i, r in enumerate(roots):
        pos[r] = (i * 3, 0)
        q.append((r, 0))
        visited.add(r)
        layers[0].append(r)

    # BFS to assign layers
    while q:
        node, depth = q.popleft()
        for child in G.predecessors(node):
            if child not in visited:
                visited.add(child)
                layers[depth + 1].append(child)
                q.append((child, depth + 1))

    # Assign x positions inside each layer
    for depth, nodes in layers.items():
        width = len(nodes)
        start_x = -1.5 * (width - 1)
        for i, node in enumerate(nodes):
            pos[node] = (start_x + i * 3, -depth)

    return pos


def draw_preimage_tree(max_depth=7):
    roots = (17)
    G = nx.DiGraph()
    q = deque()

    if isinstance(roots, int): 
        roots = (roots,)

    for r in roots:
        q.append((r, 0))
        G.add_node(r)

    #q.append((7,1))

    while q:
        y, depth = q.popleft()
        if depth >= max_depth:
            continue

        for x in preimages(y):
            G.add_edge(x, y)
            q.append((x, depth + 1))

    pos = hierarchical_layout(G, roots)

    # Color coding
    colors = []
    for node in G.nodes():
        if node == 1:
            colors.append("gold")
        # elif node in ( 4,7):
        #     colors.append("tomato")
        elif node % 2 == 0:
            colors.append("skyblue")
        else:
            colors.append("lightgreen")

    plt.figure(figsize=(24, 20))
    nx.draw(
        G, pos,
        with_labels=True,
        node_color=colors,
        node_size=900,
        font_size=8,
        arrows=True,
        arrowstyle='-|>',
        arrowsize=15,
        linewidths=1.2,
        edgecolors="black"
    )
    plt.title("Hierarchical Preimage Tree for Modified Collatz Map", fontsize=16)
    plt.show()


# Run it
draw_preimage_tree(max_depth=10)


# def f(n):
#     if n % 2 == 0:
#         return (3*n + 1) 
#     else:
#         return (n) // 2

# def preimages(y):
#     res = []

#     # odd preimage
#     x1 = 2*y - 1
#     if x1 > 0:
#         res.append(x1)

#     # even preimage
#     num = 2*y - 2
#     if num % 3 == 0:
#         x2 = num // 3
#         if x2 > 0 and x2 % 2 == 0:
#             res.append(x2)

#     return res
